...
export default function ReferralPage() {
  ...
  return (
    <div className="min-h-screen pt-24 md:pt-28 pb-24 px-4 md:px-6 max-w-3xl mx-auto">
      ...
      {/* Referral link box */}
      {user && refLink ? (
        <div className="card p-7 mb-10" style={{ border: "1px solid rgba(232,121,249,0.25)" }}>
          <p className="text-xs uppercase tracking-[0.4em] mb-4" style={{ color: "#e879f9" }}>Your Referral Link</p>
          <div className="flex gap-2 mb-4">
            <div className="flex-1 px-4 py-3 rounded-xl text-sm text-white/60 font-mono overflow-hidden text-ellipsis whitespace-nowrap"
            ...
            <button onClick={copyLink}
              ...
              style={{ background: copied ? "rgba(52,211,153,0.15)" : "rgba(232,121,249,0.12)", color: copied ? "#34d399" : "#e879f9", border: `1px solid ${copied ? "rgba(52,211,153,0.3)" : "rgba(232,121,249,0.25)"}` }}>
              {copied ? <><Check size={14} /> Copied!</> : <><Copy size={14} /> Copy</>}
            </button>
          </div>
          <p className="text-white/30 text-xs">Share this link on social media, WhatsApp, or anywhere you like.</p>
        </div>
      ) : (
        <div className="card p-8 text-center mb-10" style={{ border: "1px solid rgba(232,121,249,0.2)" }}>
          <p className="text-3xl mb-3">🔐</p>
          <h2 className="text-xl font-bold text-white mb-2">Sign in to get your referral link</h2>
          <p className="text-white/45 text-sm mb-6">Create a free account to access your unique referral link and start earning.</p>
      ...
      <div className="card overflow-hidden mb-10">
        <div className="px-6 py-4 border-b" style={{ borderColor: "rgba(232,121,249,0.1)" }}>
          <h2 className="font-bold text-white">Reward Tiers</h2>
        </div>
        {[
          ...
          <div key={i} className="flex items-center justify-between px-6 py-4 border-b last:border-0"
            ...
            <span className="text-white/70 text-sm">{r.referrals}</span>
            <span className="gradient-text font-bold text-sm">{r.reward}</span>
            {r.badge && <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: "rgba(232,121,249,0.1)", color: "#e879f9" }}>{r.badge}</span>}
          </div>
        ))}
      </div>

      <div className="text-center">
        <p className="text-white/35 text-sm mb-4">Questions about the referral program?</p>
