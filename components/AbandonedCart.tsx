...
export default function AbandonedCart() {
  ...
  return (
    <div className="fixed bottom-24 left-4 right-4 md:left-auto md:right-6 md:w-80 z-50 card p-5"
      style={{ border: "1px solid rgba(232,121,249,0.3)", boxShadow: "0 0 30px rgba(232,121,249,0.15)" }}>
      <button onClick={() => setShow(false)}
        className="absolute top-3 right-3 text-white/30 hover:text-white transition-colors">
        <X size={14} />
      </button>
      <p className="text-lg mb-1">🛒</p>
      <p className="font-bold text-white text-sm mb-1">Still thinking it over?</p>
      <p className="text-white/50 text-xs mb-4 leading-relaxed">
        Message us on Telegram — we&apos;ll hold your cart and can answer any questions you have.
      </p>
